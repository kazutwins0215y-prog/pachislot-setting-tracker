...
MAX_REQUESTS_PER_RUN = 100     # 1回の実行(メイン.py起動)で送信する最大リクエスト数の既定値。到達すると正常終了し、残りは翌回の実行が続きから自動的に再開する(2026-07-20導入・同日100へ変更。BATCH_SIZE=20+BATCH_BREAK=5分の対策で120件まで通過を確認した実績の範囲内)。CLIの`--max-requests`未指定時にresolve_max_requestsが使う既定値でもある
UNLIMITED_REQUESTS = sys.maxsize  # --max-requests 0(無制限)の内部表現。requests_remainingの減算・0以下判定にそのまま使える程度に十分大きい値にすることで、上限判定コードを分岐させずに済ませる(2026-07-22導入)
...
def process_store(con, hole_name: str, requests_remaining: int):
    ...
    try:
        for i, day in enumerate(target_days):
            ...
            try:
                day_statuses.append(_fetch_and_write(con, session, hole_name, day))
...
def _fetch_and_write(con, session, hole_name: str, day: str) -> str:
...
def resolve_max_requests(cli_value: int | None) -> int:
    """--max-requestsのCLI値を、requests_remainingの初期予算(実際に使う値)へ解決する純関数。

    未指定(None、番兵)はMAX_REQUESTS_PER_RUN(既定100)を使う(従来動作と完全一致)。
    0は「無制限」を表す(論点3・案3-1)。内部ではUNLIMITED_REQUESTS(sys.maxsize)という
    十分大きい値へ解決することで、requests_remainingの減算・`<=0`判定・スライスの
    どのコードにも無制限用の特別分岐を増やさずに済む。
    負値は誤指定として拒否する。
    """
    if cli_value is None:
        return MAX_REQUESTS_PER_RUN
    if cli_value < 0:
        raise ValueError(f'--max-requestsに負の値は指定できません: {cli_value}')
    if cli_value == 0:
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            '1回の実行で送信する最大リクエスト数。省略時は既定100(日次実行はこのまま)。'
            '0=無制限(手動フル収集用。ペース配分・サーキットブレーカーは変わらず有効)。'
        ),
    )
