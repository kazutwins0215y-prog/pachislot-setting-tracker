...
>
> **取得トランスポートはrequests方式へ復帰（2026-07-22）**: 上記のクラウド不可判定を受けてSeleniumBase (UC Mode)
> を導入したが、2026-07-22の実測で**PC上（住宅IP）でも同一条件でrequestsは200取得成功・SeleniumBaseは
> 403空ボディ**という結果が得られ、ブロック真因は「IP/Norton」ではなく「SeleniumBase(自動化ブラウザ)の
> Cloudflare指紋」だったと確定した。GitHub ActionsのIPブロックはIP/ASレベルで別問題のため変わらず不可だが、
> SeleniumBaseはクラウドでもPCでも損しかしていなかったため導入意義を失い、`scraper.py`は
> `create_session()`(requests.Session)＋`fetch_page(session, url)`へ差し替えて復帰した。
> 2026-07-20導入の安全機能（ブロック検知2層・サーキットブレーカー・リクエスト上限・ペース配分）は
> そのまま維持している。
...
| ⑤ ページ取得 | requestsでページ取得。リトライ3回・指数バックオフ、403は即abort・429/503/504は`_http_error_action`で分岐（2026-07-22導入）。**取得直後に`is_block_page`でブロックの疑いを判定**（2026-07-20追加。層1） | `scraper.fetch_page` |
...
### 取得トランスポート: requests方式（2026-07-16にSeleniumBaseへ移行→2026-07-22に復帰）

2026-07-16に「GitHub ActionsのIPがCloudflareにブロックされている」との想定でrequestsから
SeleniumBase (UC Mode)へ移行したが、2026-07-22の実測で**ブロック真因はIPではなくSeleniumBase
自体のブラウザ自動化指紋**だったと確定（同一PC・同一IPでrequestsは200取得成功、SeleniumBaseは
403空ボディ）。GitHub ActionsのIPブロックはIP/ASレベルの別問題で取得方式に関わらず解消しないため、
クラウド不可の結論自体は変わらないが、SeleniumBaseはPC実行でもブロックを誘発するだけで導入メリットが
無かったため、requests方式へ復帰した（`fase2/scrape_machine_specs.py`の`create_session`/
`_http_error_action`パターンを踏襲）。

#### `create_session()`
- `requests.Session()`を返す。User-Agent/Accept/Accept-Language/Refererヘッダーを設定
- `scrape_machine_specs.py`の`create_session`と同じヘッダー構成

#### `_http_error_action(status, attempt)`（2026-07-22導入）
- HTTPステータスコードから取るべき行動('abort'|'retry'|'raise')を返す純関数
- 403はリトライ不要で即abort（サーキットブレーカー）
- 429（レート制限）はリトライ上限到達までは'retry'、到達後は'abort'（7/14型のレート制限早期検知のため。旧SeleniumBase版・旧requests版にはこの分岐が無かった）
- 503/504は一時的エラーとしてリトライ上限到達まで'retry'、到達後は'raise'
- テスト: `tests/test_scraper_http_error_action.py`（8ケース）

#### SSL証明書検証と truststore（2026-07-14追加）
...
- `verify=False`によるフォールバックは採用しない方針（2026-07-22に再確認。中間者検知を殺さないため）

#### `fetch_page(session, url)`
- requestsでページ取得（HTML テキストを返す）
- MAX_RETRIES=3、指数バックオフ（RETRY_BASE_WAIT=60秒）でリトライ
- `_http_error_action`の判定に従い403は即`AccessForbiddenError`、429/503/504はリトライ後にabort/raise
- **取得直後に`is_block_page(html)`を呼び、ブロックの疑いがあれば`AccessForbiddenError`を送出**（2026-07-20追加。リトライしない。層1）
...
- 第1引数は常に`html`(文字列)。2026-07-16のSeleniumBase移行時も2026-07-22のrequests復帰時も
  この関数自体は変更していない（取得層の`fetch_page`がどちらの方式でも文字列を返す設計のため）
...
def main():
    ...
    max_requests = resolve_max_requests(args.max_requests)  # --max-requests未指定はMAX_REQUESTS_PER_RUN(100)、0は無制限(2026-07-22追加)
...
いずれも純関数のため`tests/test_circuit_breaker.py`でHTTP取得層/DBに触れずに単体テストできる。

### 総リクエスト上限（`MAX_REQUESTS_PER_RUN`、2026-07-20導入・`--max-requests`は2026-07-22追加）
...
- `--max-requests`（CLI・2026-07-22追加）: 省略時はデフォルト100（`resolve_max_requests(None)`が`MAX_REQUESTS_PER_RUN`を返す。従来動作と完全一致）。`0`を指定すると**無制限**（`resolve_max_requests(0)`が`UNLIMITED_REQUESTS`(`sys.maxsize`)を返す。requests_remainingの減算・`<=0`判定・スライスのどこにも無制限用の特別分岐を増やさずに済む値）。正の値はそのまま予算として使う。負の値は`ValueError`で拒否
  - 手動フル収集: `py -3.12 fase1/メイン.py --max-requests 0`（ペース配分・サーキットブレーカーは変わらず有効）
  - 日次実行（fase4）は`--max-requests`を渡さないため既定100のまま変わらない
...
### バッチ休憩・長め休憩（Cloudflare対策／累計ベース）

```python
BATCH_SIZE         = 20      # 累計この件数ごとにバッチ休憩(5分)
BATCH_BREAK        = 60 * 5
LONG_BREAK_AT      = 100     # 累計この件数ごとに長め休憩(20分)。BATCH_SIZEの倍数
LONG_BREAK_SECONDS = 60 * 20
```
...
- **休憩は「実行全体の累計リクエスト数」で判定する**（純関数`classify_pacing(global_count)`が
  `'long'`/`'batch'`/`'normal'`を返す）。`process_store`は`done_before`（この店舗開始前の累計）を
  受け取り、店舗内ループでは`global_count = done_before + i + 1`で判定。店舗の切れ目（各店の最終日は
  店舗内ループが休憩を挟まない）は`main`の`_pace_between_stores(total_done)`が1回分だけ補う。
  累計値は店舗内ループか店舗境界のどちらかでちょうど1回だけ評価され、二重発火も取りこぼしもない。
- **`LONG_BREAK_AT`(累計100件ごとに20分休憩)は2026-07-22に復活**（2026-07-20に一度削除）。
  削除時は`MAX_REQUESTS_PER_RUN=100`固定で対象日が常に100件以下に切り詰められ発火条件が
  到達不能だったが、同2026-07-22に手動フル収集用の`--max-requests 0`（無制限）を追加したことで
  1回の実行が100件を超えられるようになり、再び意味を持つようになった。日次実行（全店合計でも
  十数件＝累計<20）では`classify_pacing`が常に`'normal'`を返すため**発火せず従来通り**。

### セッション再生成（バッチ休憩後）

バッチ休憩の前に`session.close()`でTCP接続を閉じ、休憩後に`create_session()`で
新規生成する（2026-07-22のrequests復帰で「ブラウザ再生成」から戻した。requestsは
ヘッダーが同一のため指紋自体は変わらず、keep-aliveの長時間張り付きを避ける気休め程度の
効果だが、旧requests版・recover_variety_gaps.pyとの対称性のため踏襲）。処理全体は
`try/finally`で囲み、最後に必ず`session.close()`する。
...
- 依存パッケージ → `fase1/requirements.txt`（requests / beautifulsoup4 / libsql / python-dotenv / truststore。2026-07-17にrequests→seleniumbaseへ差し替え、2026-07-22にseleniumbase→requestsへ復帰）
