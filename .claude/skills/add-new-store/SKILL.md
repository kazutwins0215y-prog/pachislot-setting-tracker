...
- **`--max-requests 0`で無制限にできる**（2026-07-22追加。手動フル収集用。ペース配分・サーキットブレーカーは
  変わらず有効なため、大量バックフィルでも1回のコマンドで最後まで走らせられる。例:
  `$env:INITIAL_BACKFILL_DAYS=190; py -3.12 fase1\メイン.py --max-requests 0`。省略時は従来どおり既定100で
  複数回に分割される）
