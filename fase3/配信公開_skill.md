...
**`stage3_scores`等の列を追加・変更した場合は`--full`が必須(2026-07-08判明)**:
`upload_analysis.py`の`_ensure_remote_table`はリモートに**テーブルが存在しない場合のみ**
ローカルのCREATE TABLE文で新規作成する。既存テーブルへの列追加(`ALTER TABLE`)は
行わないため、`fase2/score.py`側で新しい列を追加してローカル`analysis.db`のスキーマだけ
マイグレーションした状態で差分upsert(`upload_analysis.py`無引数)を実行すると、
リモートの列数とズレて`SQLite error: table stage3_scores has no column named ...`で失敗する。
**サブスコア列追加などスキーマ変更を伴う改修をしたら、次のアップロードは必ず`--full`で行う**
(差分ではなく)。忘れると`fase4/run_daily.py`の日次実行が`upload_analysis`ステップで
失敗し続け(run_daily自体はERRORログのみで継続するため気づきにくい)、クラウド側
(Streamlit Cloud)のデータが更新されなくなる。
...
2. 「Create app」→ リポジトリ入力欄の**「Paste GitHub URL」**を使うのが簡単。
   ...
   ```
3. **非公開リポジトリへのアクセス権限**（2026-07-17にリポジトリをpublic化したため
   現在は通常不要。参考として残す）: 上記URLを貼ってもリポジトリが認識されない
...
| `upload_analysis.py`が`SQLite error: table stage3_scores has no column named ...`で失敗する | ローカル`analysis.db`にだけ新しい列が追加され、リモート(分析用Turso)のテーブルが追従していない状態(`_ensure_remote_table`は新規テーブル作成のみでALTER TABLEはしない)。`py -3.12 fase3/upload_analysis.py --full`で6テーブルをDROP→再作成すれば解消する(上記「`stage3_scores`等の列を追加・変更した場合は`--full`が必須」参照) |
...
| 3. 配信・公開（iPhone Web閲覧） | [`fase3/`](./) | Stage A実装済み・**Stage Bデプロイ完了(2026-07-07)** | Stage A: TursoDB移行は完了。GitHub Actionsでの自動実行はCloudflareにデータセンターIPをブロックされるため停止し、当面PC手動実行に戻した。Stage B: 分析用Turso(`pachislot-analysis`)を新設し`upload_analysis.py`（分析DB6テーブルを差分upsert）/ `analysis_turso.py`（接続ヘルパー）/ `bootstrap.py`+`streamlit_app.py`（Streamlit Community Cloud用エントリポイント）を実装、Streamlit Community Cloudへデプロイ済み。実データ約40万行の`--full`・差分upsertを実機検証済み。詳細は[`fase3/配信公開_skill.md`](配信公開_skill.md)参照 |
