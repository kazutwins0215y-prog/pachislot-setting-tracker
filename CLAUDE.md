...
| [`fase2/app.py`](fase2/app.py) | 実装済み(2026-07 UI再構成・同月モバイルUI改修) | 統合エントリポイント。ホームページ(主ページ: 店舗検索＋当日・翌日ランキング＋機能B店舗横断比較)⇔店舗トップページ(機能A店内比較＋機能B個別店舗詳細)を`st.session_state`で切替（サイドバーは廃止）。`streamlit run app.py`で起動。iPhone Safari前提のHoYoLAB風カードUI(`layout='centered'`・`ui_theme.inject_css()`・1カラムピルボタン)に改修済み |
| [`fase2/ui_theme.py`](fase2/ui_theme.py) | 実装済み(2026-07モバイルUI改修) | モバイルファーストUI(HoYoLAB風カードUI)の共通スタイル・Plotly設定を集約する表示層モジュール。`inject_css()`（角丸カード・ピルボタン・アクセント色`#4C72B0`のCSS注入）・`apply_mobile_layout(fig, height, show_colorbar)`（全チャート共通のレイアウト調整。凡例横型・`dragmode=False`）・`short_label()`を提供。機能(データ・計算ロジック)には関与しない |
