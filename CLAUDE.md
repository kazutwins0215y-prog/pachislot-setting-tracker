...
| [`fase2/app.py`](fase2/app.py) | 実装済み(2026-07 UI再構成) | 統合エントリポイント。ホームページ(主ページ: 店舗検索＋当日・翌日ランキング＋機能B店舗横断比較)⇔店舗トップページ(機能A店内比較＋機能B個別店舗詳細)を`st.session_state`で切替（サイドバーは廃止）。`streamlit run app.py`で起動 |
