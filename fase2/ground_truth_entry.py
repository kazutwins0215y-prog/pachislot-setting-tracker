...
ANNOUNCE_TO_SETTING = {'全456': '456', '全6': '6', '半数456': '456'}
...
def _scroll_to_machine_field() -> None:
    """登録完了直後に機種名フィールドが画面中央に来るようスクロールする(連続入力用)。
    標準のscrollIntoViewを使うだけなので、以前削除した自動フォーカスより壊れにくいが、
    要素の探索はStreamlitのDOM構造(data-testid)に依存する。"""
    components.html(
        """
        <script>
        (function() {
            ...
            function findMachineBox() {
                ...
                for (const el of candidates) {
                    ...
                    if (label && label.textContent.indexOf('機種名') !== -1) {
                        return el;
                    }
                }
                return null;
            ...
            // rerun直後はDOM構築中の場合があるため、見つかるまで再試行する
            let attempts = 0;
            const timer = setInterval(function() {
                attempts++;
                const box = findMachineBox();
                if (box) {
...
def main() -> None:
    ...
    if announce_type in COUNT_TYPES:
        default_count = 0
        if announce_type == '半数456' and sel_machine and con_replica is not None:
            total_units = len(fetch_units(con_replica, effective_date_str, sel_hole, sel_machine))
            if total_units:
                default_count = math.ceil(total_units / 2)
                st.caption(f'当日設置{total_units}台の半数(繰り上げ)={default_count}台を初期値にしています。異なる場合は書き換えてください。')
        numeric_value = st.number_input('数値(台数)', min_value=0, step=1, value=default_count)
    ...
    if announce_type in ANNOUNCE_TO_SETTING:
        setting_value = ANNOUNCE_TO_SETTING[announce_type]
        st.caption(f'設定値: {setting_value}(発表タイプ「{announce_type}」から自動設定)')
    else:
    ...
    if st.button('この内容で登録', type='primary'):
        if not sel_hole:
            ...
            st.session_state['last_inserted_summary'] = f'{date_str} / {sel_machine or "店舗全体"} / {announce_type}'
            st.rerun()

    summary = st.session_state.pop('last_inserted_summary', None)
    if summary:
        st.toast(f'登録: {summary}', icon='✅')
        # 連続入力しやすいよう、登録完了後に機種名フィールドへスクロールする
        _scroll_to_machine_field()
