...
Stage 1a (judge_tier / resolve_rate_columns / calibrate_all):
    機種別BB/RB/ART列のTier A/B/C判定
    (回転数チャンネルの補正はデシルカーブ方式(multi_store.py Stage1b/③ logLR_kaiten)へ統合済み。
    旧direction/strength方式の推定関数は死にコードのため2026-07-21に除去)
...
def calibrate_all(df: pd.DataFrame) -> tuple[dict, dict]:
    """
    全機種に対してTier判定・Tier B列の解決を実行する。
    ...
    return machine_tier, column_map
