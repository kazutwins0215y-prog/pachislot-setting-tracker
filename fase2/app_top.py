...
実行方法: app.py のサイドバーから選択、または `streamlit run app_top.py`
"""
import json
import sqlite3
import sys
...
def render() -> None:
    """トップページ本体。単独実行・統合アプリ(app.py)の両方から呼べる。"""
    import streamlit as st
    import plotly.express as px

    st.title('トップページ: 当日・翌日ランキング')
...
def _run_streamlit_dashboard() -> None:
    """`streamlit run app_top.py` で単独起動する場合のエントリポイント。"""
    import streamlit as st

    st.set_page_config(page_title='トップページ: 当日・翌日ランキング', layout='wide')
    render()
