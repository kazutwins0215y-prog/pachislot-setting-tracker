...
機種強さ軸・少台数機種向けFisher版 (score_zentaikei_judgment_fisher):
    機種×日×ホールでhigh_prob>=θの二値化(hot/not)を同日店内背景と比較するFisher直接確率検定。
    n=2〜4の少台数機種でscore_zentaikei_judgmentより検出力が高いことを実データで確認済み
    (今後の実装予定.md 1.8.1節、並走記録専用)
